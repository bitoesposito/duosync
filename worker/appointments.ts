...
/**
 * /api/appointments
 *   GET    ?date=YYYY-MM-DD  → all users' one-time + expanded recurring for the date
 *   POST   { userId, date?, appointment }  → upsert (one-time or recurring)
 *   DELETE ?id&isRepeating   → delete one-time appt or recurring template
 */
export async function handleAppointments(request: Request, env: Env): Promise<Response> {
