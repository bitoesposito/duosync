...
/** Reusable timeline bar that renders segments with a popover per segment. */
export function TimelineBar({
    ...
    <Popover key={segment.id}>
      <PopoverTrigger asChild>
        <div
          ...
          style={{ left: `${segment.left}%`, width: `${segment.width}%`, minWidth: "2px" }}
        />
      </PopoverTrigger>
      <PopoverContent className="rounded-none border-border w-auto p-3">
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground">{getCategoryLabel(segment.category)}</p>
        </div>
      </PopoverContent>
    </Popover>
  );
  ...
  return (
    <div
      className={`relative w-full ${height} ${borderStyle} border-border rounded-sm ${backgroundColor} overflow-hidden`}
    >
      {segments.length > 0 ? (
